...
For time-series simulations, random fading samples are drawn from Gamma distributions, scaled to match the ITU fade-depth statistics at each time percentage. Log-normal fading (small-scale, smooth envelope) is typical for terrestrial
...
ITU P.618 fade-depth reference line.
...
The total feeder link loss becomes:
\begin{equation}
L_{\mathrm{total}} = L_{\mathrm{FSPL}} + A_{\mathrm{rain}} + A_{\mathrm{cloud/fog}}
...
in \eqref{eq:feeder_loss}. Figure~\ref{fig:scint_link_loss} compares deterministic vs.\ faded feeder link loss: (a) loss envelope
...
key values at 0.1\%, 1\%, and 10\%, revealing a power budget gap that scales nonlinearly with availability target.
...
\subsection{Multi-HAPS Link and Interference Model}

The considered network contains two distinct HAPS-related communication paths:

(i) a 38~GHz HAPS-to-Gateway feeder/backhaul link, and

(ii) a relay path consisting of a HAPS-to-UAV first hop followed by a UAV-to-UE second hop.

Although both paths originate at the HAPS layer, their propagation geometry and interference conditions are different and are therefore modeled separately.

\subsubsection{HAPS-to-Gateway Feeder/Backhaul Link}

The feeder link connects each HAPS to its associated ground gateway and operates at 38~GHz. The desired received signal is generated by the serving HAPS, while transmissions from other HAPS constitute potential co-channel interference if the same time-frequency resource is reused.

For a serving HAPS $h$ and gateway $g$, the received desired power is
...
where $P\_h$ is the HAPS transmit power, $G\_h$ and $G\_g$ are the transmit and receive antenna gains, respectively, and $g\_{h,g}$ is the channel gain.

The baseline propagation loss is modeled using free-space path loss (FSPL):
...
N_g + \displaystyle\sum_{h'\neq h}
P_{h'}G_{h'}G_g g_{h',g}
},
...
N_u + \displaystyle\sum_{h'\neq h}
P_{h'}G_{h'}G_u g_{h',u}
},
...
N_k + \displaystyle\sum_{u'\neq u}
P_{u'}G_{u'}G_k g_{u',k}
},
...
Equations~\eqref{eq:feeder-sinr} and~\eqref{eq:haps-uav-sinr} are instances of a common HAPS-to-receiver SINR model. For a serving HAPS $h$ and an arbitrary receiving node $r$, the SINR is
...
N_r + \displaystyle\sum_{h'\neq h}
P_{h'}G_{h'}G_r g_{h',r}
}
\end{aligned}
}
...
10\log_{10}
\left(
\frac{P_{\mathrm{desired}}}
{P_{\mathrm{noise}}+P_{\mathrm{interference}}}
\right).
\end{equation}
...
\subsubsection{Interpretation of Multi-HAPS Interference}
...
\paragraph{Spatial geometry of the service link.}
