...
\begin{itemize}
  \item {Backhaul link: HAPS to Gateway} \\
    Backhaul link provides high-capacity connectivity between the Ground Gateway (GW) and the HAPS aircraft. Backhaul links operate in the 38.0--39.5 GHz Ka-band 3GPP NTN~\cite{3gpp_ts38821}.
 ...
 Service links are designed for connectivity from HAPS directly to  UE, ensuring accessibility without specialized hardware for a long range. Service links operate in the 2--2.2 GHz FDD band~\cite{3gpp_ts38821}.
  ...
  \item {Feeder link: HAPS to UAV} \\
    Feeder link connects the HAPS aircraft to the relaying UAV, forming the first leg of the relay path below. Feeder links operate in the 38.0--39.5 GHz Ka-band 3GPP NTN~\cite{3gpp_ts38821}, the same band as the backhaul link.

  \item {Relay Link: HAPS to UAV to User} \\
  Relay links have two legs. The first leg (the feeder link above) connects the HAPS to the UAV, and the second leg connects the UAV to the ground UEs with the UAV positioned at a lower altitude. Relay links serve as the primary backup path for users when direct HAPS-to-UE service links are unavailable. Relay links operate in 38.0--39.5 GHz Ka-band 3GPP NTN~\cite{3gpp_ts38821} for the HAPS-UAV feeder leg and the 2-2.2 GHz FDD band~\cite{3gpp_ts38821} for the UAV-User leg.
...
Backhaul links operate in the 38.0–39.5 GHz millimeter-wave band allocated to HAPS by ITU-R M.2101 (Ka-band)~\cite{itu_m2101}.
...
\begin{itemize}
  ...
  \item HAPS transmit power: $P_h = 43$\,dBm; HAPS-side transmit antenna gain: $G_h = 13.8$\,dBi (same directional Ka-band aperture as the backhaul link, \S II.B)
...
site-dependent scaling for non-uniform rain along the path). For the worst-case backhaul link (102 km slant, 11.3$^\circ$ elevation),
...
\noindent\textbf{Equation (7) --- Total Backhaul Link Attenuation~\cite{itu_p618}}: This equation sums all atmospheric and propagation effects (Eqs. 1--6) to compute the worst-case link budget for the HAPS-to-Gateway backhaul at 38 GHz. The breakdown is: FSPL (Eq. 1) + rain (Eq. 3) + fog (Eq. 4) + gas (Eq. 5) + scintillation (Eq. 6), per ITU-R P.618 design guidance~\cite{itu_p618}.
...
Table~\ref{table:app-uav-relay-comparison} (\S\ref{sec:app-uav-relay}) sweeps
the HAPS\textendash UAV horizontal distance 0--100\,km across all four
landscapes, and Figure~\ref{fig:uav-relay-comparison} plots the resulting
SINR on each hop against the direct HAPS\,$\rightarrow$\,UE link.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{./figures/12_uav_relay_comparison.png}
\caption{Direct HAPS\,$\rightarrow$\,UE SINR vs.\ the two UAV relay hops, as a function of HAPS\textendash UAV horizontal distance. The HAPS\,$\rightarrow$\,UAV feeder hop (orange) starts at $+29.3$\,dB at nadir and falls below the $+8$\,dB design floor (Eq.~\eqref{eq:sinr-min-design}) beyond $\approx45$\,km, crossing 0\,dB near 60\,km; the UAV\,$\rightarrow$\,UE access hop (green band, suburban\textendash high-rise range) stays well above it throughout.}
\label{fig:uav-relay-comparison}
\end{figure}

\noindent\textbf{Key finding: the feeder hop, not the access hop, bottlenecks this relay
at long range, but is strongly positive near nadir.}
Because the UAV hovers directly above the user it serves (\S II), its
horizontal distance from the user is small and fixed while its horizontal
distance from the serving HAPS grows with the user's own distance from the
HAPS nadir. The UAV\,$\rightarrow$\,UE hop therefore stays strong
($\mathrm{SINR}\approx+29$ to $+31$\,dB across all four landscapes,
Table~\ref{table:app-uav-relay-comparison}) regardless of where the UAV is
deployed. The HAPS\,$\rightarrow$\,UAV hop, by contrast, is a 38\,GHz slant
path whose elevation angle falls with horizontal distance, so both free-space
loss and the elevation-dependent ITU-P.618 scintillation margin
(\S\ref{subsec:scintillation}) worsen together. Earlier revisions of this
analysis evaluated that hop with $P_{\text{TX,HAPS}}$ only and \emph{no}
antenna gain, which put the SINR below $-15$\,dB even at nadir -- physically
unrealistic for a point-to-point Ka-band link, which is always closed with
directional antennas rather than raw transmit power. Restoring the
transmit/receive antenna-gain terms $G_h$, $G_u$ that the general SINR
expression already calls for (\S III, Eq.~\eqref{eq:haps-uav-sinr}), using
literature-typical gains for this class of link -- $G_h=13.8$\,dBi HAPS-side
and $G_u=30.7$\,dBi UAV-side, matching a published 30\,GHz HAPS backhaul-class
budget (Tx power 43.2\,dBm, essentially identical to
$P_{\text{TX,HAPS}}=43$\,dBm used throughout this document) and a
UAV-mountable mmWave patch-array antenna,
respectively~\cite{karaman2025haps,anim2021uav} -- moves the HAPS\,$\rightarrow$\,UAV
SINR to $+29.3$\,dB at nadir, clearing the $+8$\,dB design floor
(Eq.~\eqref{eq:sinr-min-design}) out to $\approx45$\,km, staying positive out
to $\approx60$\,km, and matching the direct link's $-5$\,dB floor out to
$\approx70$\,km, before falling to $-20.6$\,dB at the 100\,km sweep edge as
the elevation angle drops to $11.0^\circ$ and scintillation margin grows.
Consequently $C_{\text{uav-relay}}$ now reaches
$9.7$\,bits/s/Hz at nadir -- competitive with, and at some landscapes
exceeding, the direct HAPS\,$\rightarrow$\,UE link's $1.9$--$6.2$\,bits/s/Hz
over the same range (Table~\ref{table:app-uav-relay-comparison}) -- and
collapses toward the UAV\,$\rightarrow$\,UE hop's ceiling only where the
feeder hop's low elevation angle makes it the bottleneck. This quantifies
the near-nadir placement guidance already noted for the relay UAV (\S II):
the UAV relay is capacity-competitive with the direct link only within
roughly the inner half of the service footprint, where its HAPS-facing
feeder hop still clears the design SINR; beyond that, the direct link
remains the better choice. The DQN association agent
(\S\ref{sec:phase2-motivation}) does not currently route traffic through the
UAV relay; extending its action space to do so conditionally (e.g.\ only
within the feeder hop's viable range) is noted as future work in the
Conclusion (\S\ref{sec:conclusion}).
...
Figure~\ref{fig:sinr-feeder-link} shows the 38~GHz backhaul-link SINR for gateway offsets of 10--500~m from nadir (gateway at 50\,m altitude, HAPS at nominal stratospheric altitude), including the HAPS-side and Gateway-side antenna gains of \S II.B ($G_h=13.8$\,dBi, $G_g=39.7$\,dBi). The backhaul slant range is set by the HAPS altitude ($\sim$20\,km), against which a 10--500\,m horizontal offset is negligible, so SINR is essentially flat at $\approx+38.0$--$38.1$\,dB across this range -- strongly positive once the directional antenna gains are included, versus the deeply negative result obtained when the link budget used raw transmit power alone (\S II.B). The interfering HAPS, by contrast, sit at a slant range of $\sim$65--68\,km (equilateral-triangle geometry, \S III.A); the resulting interference power is $\approx-43$\,dB below the noise floor. The backhaul link is therefore \textbf{noise-limited, not interference-limited}, unlike the service link.

\begin{figure}[h!]\centering
\includegraphics[width=0.95\columnwidth]{./figures/haps_feeder_link_38ghz.png}
\caption{38~GHz backhaul link (HAPS-to-Gateway) SINR, four identical landscape subplots (landscape-independent, \S III.C), 10--500\,m distances.}
...
profile and discharged by a constant platform/backhaul draw plus its currently-associated
...
\noindent \textbf{Backhaul link (38 GHz) does NOT experience this penalty}: The three interfering HAPS are $\approx 65$ km from the ground Gateway, placing them far outside the 10--500 m backhaul coverage. Consequently, interference from constellation members remains well below thermal noise floor; backhaul SINR is primarily noise-limited and approximately constant at $+38$~dB, once the HAPS/Gateway directional antenna gains (\S II.B) are included in the link budget (Section~\ref{subsec:feeder-sinr}, Figure~\ref{fig:sinr-feeder-link}).

\paragraph{Scintillation fading (ITU P.618 implementation).}
Figure~\ref{fig:scint_fade_depth} introduces a physics-based model of tropospheric scintillation affecting both service links (UE $\leftrightarrow$ HAPS, 5$^\circ$--85$^\circ$ elevation-dependent) and backhaul links (HAPS $\leftrightarrow$ Gateway, 25$^\circ$--45$^\circ$ fixed). The fade-depth model
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item \textbf{Included}: Deterministic SINR from constellation geometry (Phase~1); rainfall \& visibility for backhaul margin planning; battery state for load balancing
  ...
  \item \textbf{Approximation}: Phase~2 assumes worst-case static fading margin ($3.65$~dB service, $10.53$~dB backhaul at $p=1\%$) or uses Phase~1 fade-depth statistics as penalty modulation
...
model to both characterise multi-HAPS interference (\S\ref{sec:phase1-summary})
and train a DQN agent for user-platform association (\S\ref{sec:phase2-results}).
...
(\S\ref{sec:phase1-summary}) exceeds the degradation from doubling distance.
...
margin on both links (\S\ref{subsec:scintillation}).
...
evaluated as an alternative to the direct service link. (The
HAPS$\rightarrow$Gateway link, \S III.B, is this network's fixed backhaul to
the Internet, not a relay to the UE -- the Gateway is not evaluated as a
relay path here.) For the UAV relay, the access hop (UAV$\rightarrow$UE) is
consistently strong ($+29$ to $+31$\,dB SINR) because the UAV hovers close to
the user it serves, while the feeder hop (HAPS$\rightarrow$UAV) is a
38\,GHz slant path whose SINR falls with distance as elevation angle drops
and the ITU-P.618 scintillation margin grows. With literature-typical
antenna gains restored on this hop ($G_h=13.8$\,dBi HAPS-side,
$G_u=30.7$\,dBi UAV-side~\cite{karaman2025haps,anim2021uav} -- the general
SINR expression, \S III, already calls for these gain terms, but they were
previously omitted from the numeric link budget), SINR starts at
$+29.3$\,dB at nadir and clears the $+8$\,dB design floor
(Eq.~\eqref{eq:sinr-min-design}, derived from the 16-QAM/CQI-7 minimum-MCS
SINR at 10\% BLER~\cite{3gpp_ts38214} plus a 3\,dB margin) out to
$\approx45$\,km, before falling below the direct link's own $-5$\,dB floor
beyond $\approx70$\,km as the path approaches the 100\,km sweep edge
($11.0^\circ$ elevation). The decode-and-forward end-to-end capacity
therefore reaches $9.7$\,bits/s/Hz at nadir -- competitive with the direct
link's $1.9$--$6.2$\,bits/s/Hz over the same range (Table~\ref{table:app-uav-relay-comparison})
-- and is feeder-hop-limited only in the outer half of the service
footprint. The relay is thus capacity-competitive with the direct link
within roughly the inner $\approx45$--$60$\,km of HAPS nadir, but not
beyond; this range-dependence, rather than blanket non-viability, is why the
Phase~2 DQN association agent (\S\ref{sec:phase2-motivation}) still
restricts its action space to the three direct HAPS choices and does not yet
route through the UAV relay -- doing so conditionally on distance is noted
as future work below.

\paragraph{Learned association.} The DQN agent trained on the same
environment as the closed-form baselines (\S\ref{sec:phase2-results})
substantially outperforms random association (reward $-34.1$ vs.\ $-82.2$,
outage $9.8$ vs.\ $33.7$ users out of 50) but does not yet match the
closed-form max-SINR heuristic (reward $-19.3$, outage $1.0$). This gap,
combined with the UAV relay's distance-dependent viability, motivates two
directions for future work: (1) closing the DQN-vs-max-SINR gap through
extended training, reward shaping, or harder (e.g.\ rain/mobility-heavy)
scenarios, and (2) extending the DQN's action space to include UAV relay
routing alongside direct association -- now that the feeder hop is
capacity-competitive within $\approx45$--$60$\,km of nadir -- so the learned
policy, rather than a fixed three-way choice, decides when relaying is
